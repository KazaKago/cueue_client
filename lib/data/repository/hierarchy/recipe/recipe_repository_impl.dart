...
class RecipeRepositoryImpl implements RecipeRepository {
  const RecipeRepositoryImpl(this._getRecipesApi, this._getRecipeApi, this._createRecipeApi, this._updateRecipeApi, this._deleteRecipeApi, this._getMenuApi, this._getMenusApi, this._recipeResponseMapper, this._recipeSummaryResponseMapper, this._recipeRequestMapper, this._menuSummaryResponseMapper, this._menuResponseMapper);
  ...
  LoadingStateStream<List<RecipeSummary>> followAllData() {
    final allRecipeFlowable = AllRecipesFlowableFactory(_getRecipesApi, _recipeSummaryResponseMapper).create();
  ...
  LoadingStateStream<List<RecipeSummary>> followTaggedData(final TagId tagId) {
    final taggedRecipeFlowable = TaggedRecipesFlowableFactory(_getRecipesApi, _recipeSummaryResponseMapper, tagId).create();
  ...
  LoadingStateStream<Recipe> followData(final RecipeId recipeId) {
    final recipeFlowable = RecipeFlowableFactory(_getRecipeApi, _recipeResponseMapper, recipeId).create();
    return recipeFlowable.publish();
  }

  @override
  Future<void> refreshAllData() async {
    final recipeFlowable = AllRecipesFlowableFactory(_getRecipesApi, _recipeSummaryResponseMapper).create();
  ...
  Future<void> create(final RecipeRegistration recipeRegistration) async {
    ...
    final recipeFlowable = RecipeFlowableFactory(_getRecipeApi, _recipeResponseMapper, recipe.id).create();
    ...
    for (final tag in recipe.tags) {
      final taggedRecipeFlowable = TaggedRecipesFlowableFactory(_getRecipesApi, _recipeSummaryResponseMapper, tag.id).create();
  ...
  Future<void> update(final RecipeId recipeId, final RecipeRegistration recipeRegistration) async {
    ...
    RecipeCache.sharedInstance.taggedRecipeIds.forEach((key, value) async {
      if (!recipe.tags.map((e) => e.id).contains(key)) {
        final taggedRecipeFlowable = TaggedRecipesFlowableFactory(_getRecipesApi, _recipeSummaryResponseMapper, key).create();
        final cachedTaggedRecipeIds = await taggedRecipeFlowable.getData(from: GettingFrom.cache);
        if (cachedTaggedRecipeIds != null) {
